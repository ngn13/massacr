...
char *bad_ranges[] = {
    ...
    "192.0.0.8/32",   "192.0.0.9/32",    "192.0.0.170/32",    "192.0.0.171/32",
...
bool get_range(range_t *res, char *ipnet) {
...
bool skip_range(range_t *ranges, uint8_t *cur) {
  for (int i = 0; i < bad_range_size; i++) {
    if (ranges[i].net == 8 && ranges[i].ip[0] == cur[0])
      return true;

    else if (ranges[i].net < 8 && ranges[i].ip[0] <= cur[0])
      return true;

    else if (ranges[i].net == 16 && ranges[i].ip[0] == cur[0] &&
             ranges[i].ip[1] == cur[1])
      return true;
    ...
    else if (ranges[i].net == 24 && ranges[i].ip[0] == cur[0] &&
             ranges[i].ip[1] == cur[1] && ranges[i].ip[2] == cur[2])
      return true;
    ...
    else if (ranges[i].net == 32 && ranges[i].ip[0] == cur[0] &&
             ranges[i].ip[1] == cur[1] && ranges[i].ip[2] == cur[2] &&
             ranges[i].ip[3] == cur[3])
      return true;
